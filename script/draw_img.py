...
# def setpic(point, pixv):  # 画素値の設定
#     # 直線が通過するごとに1だけ増やす
#     # pix[tuple(point)] = np.min([pix[tuple(point)]+1, maxpix])
#     # pix[tuple(point)] = 1  # 2値画像
#     pix[tuple(point)] = pixv  # グラデーション

def drawline(from_p,  dest_p, pixv):  # from_p-dest_pを結ぶ直線を描画する
    # 画素値 pixv を引数として渡すように改変
...
#if (rat := np.max([width/imgw, height/imgh])) > 1.0:  # 描画領域を超えている
...
for i in range(len(dat)-1):
    drawline(dat[i], dat[i+1], math.floor((maxpix+1)/len(dat)*i) )
    #drawline(dat[i], dat[i+1], 1)
...
for row in np.flipud(pix.T):   # pix を転置して縦方向に反転（配列の並びの関係で必要）
    print(' '.join(np.array(row, dtype=str)))
    # print(''.join(np.array((row+1)%2, dtype=str)))  # 反転表示：2値画像
